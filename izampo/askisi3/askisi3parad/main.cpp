...
int main(int argc,char*argv[]){
	...
	int choose,counter;
	double total_charge,total_charge2;
	int N, M;
	M=atoi(argv[1]);
	N=atoi(argv[2]);
	vector<particle*>my_array(M);
	int total_counter = 0;
	for (int i = 0; i < M; i++){
		choose = rand() % 7 + 1;
		if (choose == 1){
			my_array[i]= new proton();
		}
		else if (choose == 2){
			my_array[i]= new neutron();
		}
		else if (choose == 3){
			my_array[i]= new electron();
		}
		else if (choose == 4){
			my_array[i]=new positron();
		}
		else if (choose == 5){
			my_array[i]=new neutrino();
		}
		else if (choose == 6){
			my_array[i]=new antineutrino();
		}
		else if(choose==7){
			my_array[i]=new photon();
		}
	}
		counter = 0;
	for (int i = 0; i < M; i++){
